...
def convert_to_mp4(source: Path) -> Path:
    ...
    We always re-encode instead of skipping existing .mp4 files, because
    the container might still use codecs that iOS cannot edit reliably
    (e.g. HEVC, unusual audio codecs, etc.).
    """
    suffix = source.suffix.lower()

    # Always write to a new file so we don't overwrite the source in-place
    if suffix == '.mp4':
    ...
    if result.returncode != 0:
        raise RuntimeError(f"ffmpeg failed: {result.stderr.decode('utf-8', errors='ignore')[:400]}")
    return target
...
def download_video(message, url, audio=False, format_id="bestvideo+bestaudio"):
    if not url:
    ...
    if config.max_filesize and not nextcloud_enabled():
    ...
    # Force iPhone-friendly video format (H.264 + AAC in MP4) for video downloads.
    # We keep custom /custom selections but always fall back to safe MP4.
    if not audio:
        preferred_format = (
        ...
        # If user didn't request a specific format, use our preferred chain directly.
        if format_id == "bestvideo+bestaudio":
            ydl_opts['format'] = preferred_format
        else:
            # Prefer the user-selected format, but fall back to our MP4 chain.
            ydl_opts['format'] = f"{format_id}/{preferred_format}"

        # Ensure merged output is MP4 so Telegram/iOS sees a standard container.
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:  # type: ignore[arg-type]
            ...
            bot.edit_message_text(
                chat_id=message.chat.id, message_id=msg.message_id, text='Processing file with ffmpeg...')
            ...
            if nextcloud_enabled():
                bot.edit_message_text(
                    chat_id=message.chat.id,
                    message_id=msg.message_id,
                    text='Uploading to Nextcloud...'
                )
                share_url = upload_to_nextcloud(final_file)
                safe_unlink(final_file)
                final_file = None
                ...
                with final_file.open('rb') as f:
                    if audio:
                        ...
                        bot.send_video(message.chat.id, f, reply_to_message_id=message.message_id, width=width, height=height)
